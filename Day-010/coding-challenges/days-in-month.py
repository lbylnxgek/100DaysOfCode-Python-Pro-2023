...
def is_leap(year):
    """Return True if year is a leap year or False if not"""
    if year % 4 == 0:
        if year % 100 == 0:
            if year % 400 == 0:
                return True
...
def days_in_month(year, month):
    """Return the number of days for a year and month combination
    taking into account leap years, in which February has 29 days."""
...
def month_number_to_name(month_num):
    """Take a month number and return the month name."""
    month_name = [
        "January",
        "February",
        "March",
